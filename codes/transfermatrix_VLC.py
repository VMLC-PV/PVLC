...
#The original code by George F. Burkhard, Eric T. Hoke, Stanford University and 
#Kamil Mielczarek (kamil.m@utdallas.edu), University of Texas at Dallas is modified to 
# be used in combination with the DDSuite by written by Prof. Jan Anton Koster (https://github.com/kostergroup).
# These small modifications are made by V.M. Le Corre (https://github.com/VMLC-PV) 
...
def get_ntotal(matName,matDir,lambdas,matPrefix = 'nk_',matHeader = 1):
    """ Get complex refactive index from material file
    ...
    matDir : str
    Material nk file location

    lambdas : 1-D sequence of floats
    Wavelengths

    matPrefix : str
    materials data prefix, default 'nk_'

    matHeader : int
    number of lines in header, default 1

    Returns
    -------
    1-D sequence of complex
    Complex refactive index
    """
...
def transfermatrix(genrate_profile_name='generation_profile.txt',layers = [],thicknesses = [],activeLayer = 3,posFrontTL = 2, posBackTL = 4,polarity='pin',lambdas=np.arange(350,800,1),Makeplot = True,plotWavelengths = [400 , 500 , 600],x_step=1.0,matDir= 'matdata_VLC',matPrefix = 'nk_',matHeader = 1,num_fig=0):
    ...
    layers : list, optional
        list of the layers names, by default []

    thicknesses : list, optional
        list of the layers thicknesses in nm, by default []

    activeLayer : int, optional
        Active layer position, indexing starts from 0, by default 3

    posFrontTL : int, optional
        Front TL position, if no front TL use same as active layer, indexing starts from 0, by default 2

    posBackTL : int, optional
        Front TL position, if no back TL use same as active layer, indexing starts from 0, by default 4

    polarity : str, optional
        device polarity 'pin' pr 'nip', by default 'pin'

    lambdas : np.array, optional
        array with the wavelengths raneg to calculate, by default np.arange(350,800,1)

    Makeplot : bool, optional
        Make plots, True/False, by default True

    plotWavelengths : list, optional
        Wavelengths to plot the E-field, by default [400 , 500 , 600]

    x_step : float, optional
        grid spacing of device cross section simulation in nm, by default 1.0

    matDir : str, optional
        materials data folder, by default 'matdata'

    matPrefix : str, optional
        materials data prefix, by default 'nk_'

    matHeader : int, optional
        number of lines in header of material data files, by default 1

    num_fig : int, optional
        start numbering plots, by default 0
    """
        ## USER CONFIGURATION
    # Build up the device, light enters from left to right, the layer names
    # correspond to the csv filenames in the 'matDir' ignoring the material prefix.
    ...
    if lambdas.size == 0:
        ...
        lambdas = np.arange(lambda_start,lambda_stop+lambda_step,lambda_step)
    ##
    ## START OF CODE. DO NOT EDIT FURTHER.
    ##

    # Constants
    h = constants.value(u'Planck constant') # Js Planck's constant
    c = constants.value(u'speed of light in vacuum') # m/s speed of light
    q = constants.value(u'elementary charge')#C electric charge
    ...
    for i,l in enumerate(layers):
        ni = np.array(get_ntotal(l,matDir,lambdas,matPrefix,matHeader))
        n[i,:] = ni
    ...
    devpos= np.asarray([])
    idx = 0
    for i in np.arange(posFrontTL,posBackTL+1,1):
        Pos = np.nonzero(x_mat == i)
        for j,k in enumerate(Pos[0]):
            if i == activeLayer:
                genrate.append(Gx[0][idx])
                idx = idx+1
            else:
                genrate.append(0)
        devpos= np.append(devpos,Pos)

    
    devpos = devpos - min(devpos)
    if polarity == 'pin':
        genrate = genrate[::-1]
    elif polarity == 'nip':
        pass
    else:
        sys.exit('Wrong polarity input, need to be \'nip\' or \'pin\' so the function transfermatrix was exited early!')

    genrate = np.asarray(genrate)*1e6
    genrate_profile = pd.DataFrame({'x':devpos,'Gehp':genrate})
    genrate_profile.to_csv(genrate_profile_name,sep=' ', index=False,float_format='%.3e',header=False)
...
if __name__ == '__main__':
    ...
    transfermatrix(layers = ['SiO2Brabec' , 'ITOBrabec' , 'PEDOT' , 'PTO2', 'IDTT4FIC', 'DPO' , 'AgBrabec'],thicknesses= [0 ,110 ,40  , 50, 60 , 10 , 200],lambdas=np.arange(350,800,1),polarity='nip',activeLayer=3)
