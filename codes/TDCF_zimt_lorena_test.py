...
import os,sys,platform,tqdm,parmap,multiprocessing
import platform
import numpy as np
import pandas as pd
...
def TDCF(L,num_fig,str2run='',path2ZimT='',Store_folder=''):
    """Run TDCF simulations

    Parameters
    ----------
    L : float
        Device total thickness (m).

    str2run : str, optional
        Input string to run with ZimT (Defaults = '').

    path2ZimT : str, optional
        Path to folder containing ./ZimT or zimt.exe in current directory (Defaults = '').

    Store_folder : str, optional
        Path to folder where the ouput of the simulation is stored in path2ZimT directory(Defaults = '').
    ...
    tcol = 40e-6                                                 # tmax = tcol + tdelay  = final time step
    Vpres = [-0.5,0,0.2,0.4,0.6,0.7,0.8,0.85,0.9,1]#,0.85]      # Initial applied voltage (steady-state)
    Vcol = -2.5
    ...
    if run_simu:
        ...
        for Vpre in Vpres:
            for tdelay in delays:
                ...
                str_lst.append('-L '+str(L)+str2run+' -tVG_file tVG_TDCF_dark_delay_{:.1e}_Vpre_{:.2f}.txt -tj_file tj_TDCF_dark_delay_{:.1e}_Vpre_{:.2f}.dat'.format(tdelay,Vpre,tdelay,Vpre))
            ...
            for Gen in Gens:
                for tdelay in delays:
                    str_lst.append('-L '+str(L)+str2run+' -tVG_file tVG_TDCF_Gen_{:.1e}_delay{:.1e}_Vpre_{:.2f}.txt -tj_file tj_TDCF_Gen_{:.1e}_delay{:.1e}_Vpre_{:.2f}.dat'.format(Gen,tdelay,Vpre,Gen,tdelay,Vpre))
    ...
    for Vpre in Vpres:
        for tdelay in delays:
            ...
            for Gen in Gens:
                ...
                npre_dumb.append(abs(simps(pre['Jtdcf']/(q*L),x=pre['t'])))
                ncol_dumb.append(abs(simps(col['Jtdcf']/(q*L),x=col['t'])))
                ntot_dumb.append(abs(simps(data_tj['Jtdcf']/(q*L),x=data_tj['t'])))
                ...
                if plot_extract_charges:
                    ...
                    # extrat_charge = integrate.cumtrapz(col['Jtdcf'], col['t'], initial=0)
                    # col['Qext']= extrat_charge/(q*L)
                    # col['t'] = col['t']-min(col['t'])
                    # plt.semilogx(col['t'],col['Qext'],color=colors[idx],label='Vpre = {:.2f}'.format(Vpre))
    ...
    if plot_extract_charges:
        plt.figure(num_fig_extract_charges)
        # plt.vlines(L**2/(2*1e-7*1.4),1e21,2e22,label='t$_{max}$',colors='r')
        # plt.vlines(L**2/(2*1e-9*1.4),1e21,2e22,label='t$_{min}$',colors='b')
...
if __name__ == '__main__':
    ...
    for ratio in ratios:
        ...
        for Lang_pre in Lang_pres:
            ...
            # print('mu {:.1e} Lang_pre {:.1e} and Ratio {:.0f} is done!'.format(mumax,Lang_pre,ratio))
            plt.figure(index)
            plt.plot(Vpres,Qext_Qgen,label='{:.1e}'.format(Lang_pre))
        plt.grid(b=True,which='both')
        plt.xlim(min(Vpres)-0.1,max(Vpres)+0.1)
        plt.ylim(0,1.1)
        plt.legend(loc='best',title='$\gamma_{pre}$')
        plt.xlabel('V$_{pre}$ [V]')
        plt.ylabel('Q$_{ext}$/Q$_{gen}$')
        plt.tight_layout()
        plt.savefig('chargevsvoltage_ratio_{:.0f}_profile_G_1e21.pdf'.format(ratio),dpi=300,transparent=True) 
        # plt.savefig('chargevsvoltage_ratio_{:.0f}_profile_TL.jpg'.format(ratio),dpi=300,transparent=True) 
        # plt.close()
        plt.show()
        index = index + 1